...
**Active milestone: v1.57 — Admin Operator Control Plane (SEED-004, M1)** (opened 2026-07-19). Opens the SEED-004 admin redesign, starting with the **M1 IA/grammar pivot**. **v1.56 (Admin UI Ratchet) is PARKED** — phases 205–207 shipped, 208 was 3/5 with 208-04/05 maintainer-gated and **non-converging because the ratchet kept surfacing information-architecture findings** (23 confirmed in round 99). That is exactly the work v1.57 now takes on, so parking (not abandoning) is the right call: the ratchet ledger/baseline in `accrue_admin/e2e/ratchet/` are preserved and become the tool that re-locks the v1.57 redesign once it lands. Immediately preceding v1.57, a direct de-garish + card-grammar polish pass (quick tasks 260718-u1s / 260719-ey5 / 260719-fz6) already made the admin visually coherent; v1.57 M1 does the IA/component-cohesion layer on top.
...
**v1.57 = SEED-004 M1 (admin-only IA/grammar pivot).** First-principles move of `accrue_admin` toward an "operator control plane over billing state": one health verdict + one primary action per zone, trim redundant bands, reign Home + Subscriptions onto the shared component vocabulary (PageHeader / StatStrip / DataTable / FilterChipBar / `.ax-card` / Button / StatusBadge / EmptyState). Driven by the 23 round-99 ratchet-confirmed IA findings (`research/admin-ratchet-round99-confirmed-findings.json`). **M2** (signature "Why blocked?" diagnosis surfaces + causality graph + core `accrue` diagnosis fns) and **M3** (new Usage/checkout/fee-reconciliation rooms + ratchet re-freeze) are later milestones. North-star: `prompts/accrue_admin_operator_ui_journey_blueprint.md`; synthesis: `.planning/research/ADMIN-UI-REDESIGN-BLUEPRINT-SYNTHESIS.md`; seed: `.planning/seeds/SEED-004-admin-ui-blueprint-redesign.md`. Reopen class: explicit strategy change — flagship admin surface.

## Current Milestone: v1.57 Admin Operator Control Plane (SEED-004, M1)

**Goal:** Begin the SEED-004 redesign of `accrue_admin` from a CRUD surface into an "operator control plane over billing state," via the **M1 information-architecture / grammar pivot** — one health verdict + one primary action per zone, trim the redundant bands, and reign the two outlier pages (Home + Subscriptions) onto the same shared component vocabulary as the rest of the admin — so the whole admin reads as one cohesive, operator-first system.

**Target features (M1 scope):**
- Reign **Home** and **Subscriptions** onto the canonical shared vocabulary (PageHeader / StatStrip / shared DataTable + cell idiom / FilterChipBar / `.ax-card` / Button / StatusBadge / EmptyState), retiring the bespoke `.ax-home-*` / `.ax-launcher*` / `.ax-attention*` / `.ax-subscriptions-*` / `.ax-inline-worklist*` component sets (~325 rules).
- IA pivot on those pages: one scannable health verdict, one clear primary action per zone, redundant/duplicate bands trimmed (informed by the 23 round-99 ratchet-confirmed findings).
- A consistent "answer-first" screen grammar the other pages already exhibit, applied to the outliers → a proper cohesive baseline before M2's signature diagnostic surfaces.

**Key context / constraints:** Reopen class = explicit strategy change (flagship admin surface). Scope this milestone is **admin-only** (`accrue_admin` templates + `assets/css`), no core `accrue` change (M2 is where core diagnosis fns like `blocking_reason_for_owner/1` come in), no `accrue_portal` work, `ax-*` stays styling SSOT, keep the Cobalt/quiet-confidence brand + prior polish passes. Reuse the shared components as-is (compose, don't fork); one small new shared component allowed only if a work-queue "callout" shape clearly repeats. Edit source + rebuild the committed `priv/static/accrue_admin.css` bundle; PNG-verify against the canonical Payments/Customers/Invoices reference. The parked v1.56 ratchet is the tool that will re-lock this redesign (rubric/exemplar refresh + baseline re-freeze) after M1 lands. Authoritative design sources: `prompts/accrue_admin_operator_ui_journey_blueprint.md`, `.planning/research/ADMIN-UI-REDESIGN-BLUEPRINT-SYNTHESIS.md`, `research/admin-ratchet-round99-confirmed-findings.json`.
...
---
*Last updated: 2026-07-19 — Phase 209 (v1.57 Admin Operator Control Plane M1, Reign Subscriptions) complete; v1.56 Admin UI Ratchet parked mid-flight. Prior: opened v1.56 on 2026-07-03.*

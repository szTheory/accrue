...
Current focus: **v1.39 — Entitlements / Plan-Gating** (opened 2026-05-22). **Phase 123 (Config + Core Gate API Foundation) complete 2026-05-23** — ENT-01..05 validated (`123-VERIFICATION.md`, 5/5): a boot-validated `:entitlements` plan→feature/quota config, the four fail-closed `Accrue` gate functions (`has_active_plan?` / `entitled?` / `features_for` / `entitlement_quantity`) resolved entirely from local subscription state with zero processor calls, and the `[:accrue, :entitlements, :check]` telemetry/ledger split. Phase 124 (enforcement surfaces — Plug + LiveView guards) is next. Last shipped: **v1.38** — current public linked release line accrue / accrue_admin / accrue_portal `1.1.1` (published 2026-05-08; tags since moved to `1.1.2`). The v1.38 post-publish closeout (planning mirrors + `INV-08`) is complete.

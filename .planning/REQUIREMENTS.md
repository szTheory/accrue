...
- [ ] **DUN-01**: A host can configure a multi-step dunning cadence (ordered steps, each with a delay and an email template) via `NimbleOptions`-validated config nested under the existing `:dunning` key, with a sensible default journey shipped out of the box (opt-out, not opt-in).

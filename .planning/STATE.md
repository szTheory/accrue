...
| 260622-nob | Finish host Playwright e2e CI green-up (2 commits, `host-integration` + `playwright-e2e` jobs). Repointed apply-filters button assert (now `ax-visually-hidden`, auto-applies) → `[data-role='filter-form']`; repointed phase13 dead `getByText("Append-only billing and admin activity")` → `getByRole('heading',"Event log")` (root cause of "Target page closed" + Ecto `Sandbox :checkin` crash). Local full-suite run surfaced 2 more drifts the handoff missed: regenerated the STALE committed `e2e/generated/copy_strings.json` (5 fql headline keys; `host-integration` regenerates at runtime but `playwright-e2e` shard reads it committed) + repointed customers index "Billing signals" columnheader → "Payment method" (column removed in redesign). Verified locally: `npx playwright test` 19 passed / 3 skipped / 0 failed. `mix.lock` deliberately left untouched (committed hex-mode lock required by `accrue_host_hex_smoke.sh`; deps.get rewrote it locally, restored after). | 2026-06-22 | 0a2f25ba | [260622-nob-fix-two-host-playwright-e2e-spec-drift-a](./quick/260622-nob-fix-two-host-playwright-e2e-spec-drift-a/) |
...
| 260622-h7h | Green the package-docs CI gate — fixed a STACK of accumulated app.css doc-contract violations (started as 1-line DSY-01, unmasked 2 more; `verify_package_docs.sh` exits on first failure). All introduced by pushed-backlog tasks since last-green `096256d0`, invisible locally (CI-only bash gate + core PackageDocsVerifierTest, neither runs in `cd accrue_admin && mix test`). (1) **DSY-01** (theme-picker ec45880e): `@media (max-width:767px)` missing `--ax-bp-*` annotation → `767.98px /* --ax-bp-md ↓ */`. (2) **FND-02** (timeline ps2): rail/node `z-index:0/1` micro-stack missing `ax-z-micro-stack` annotations + `isolation:isolate` owner → annotated both + added `.ax-timeline-item{isolation:isolate}`. (3) **FND-01** (h72 `.ax-kpi-meta`, MY olr `.ax-stat-value`, ps2 `.ax-timeline-time`): raw type decls outside allowlist → block-scoped `ax-type-exception` comments. Two are functional (767.98 + isolation) so REBUILT committed bundle (js unchanged). verify_package_docs.sh full PASS; core PackageDocsVerifierTest 29/0; admin assets_test 3/0; compile clean. Greens the gate in all 3 CI places (bash job + core test + Release-gate matrix). REMAINING main-CI red: Phase 190/191/192 Playwright browser gate (deterministic copy/DOM drift from the UI redesign) — separate debug next. Lesson: doc/CSS changes need `verify_package_docs.sh` run locally | 2026-06-22 | 60fb1c06 | [260622-h7h-dsy01-theme-picker-media-annotation](./quick/260622-h7h-dsy01-theme-picker-media-annotation/) |
| 260622-i2c | Green the accrue_admin Playwright browser UAT — the only remaining main-CI red after the doc-gate fix (also folded into the Release gate via the Phase 192 step). Failures were PRE-EXISTING e2e spec-drift from the redesign arc (h72 webhooks selection-retry → io6 customers filter removal → olr stat-strip → fql headers) that shipped without updating the Playwright specs (every task ran only `mix test`, never the UAT) + 1 real responsive bug. fql did NOT cause it (fql FIXED the `/Invoices/i` probe). Iterated LOCALLY (config auto-boots `mix accrue_admin.e2e.server`; chromium installed) → **92 passed/0 failed** both chromium-desktop+mobile, every spec. **1 real APP FIX**: `.ax-kpi-delta{white-space:nowrap}` forced customer-detail ~34px past 320px → `white-space:normal;overflow-wrap:anywhere` + `.ax-kpi-card{min-width:0}`, rebuilt bundle (helper NOT loosened). **8 spec-drift realigns** (none weakened/skipped): webhooks bulk-replay `prepare-bulk-replay`→`toggle-all`/`bulk-action`/`confirm-retry-selected` + new copy (phase191:307, phase7-uat); customers state-copy `?status=no-records`→`?q=<no-match>` empty-state (filter removed by io6); dashboard h1/brand/theme-radio drift; admin-baseline mobile 240s timeout restore; focus-ring probe → forced-focus specimens (WR-07); motion-trace `data-collapse-toggle`→sidebar overlay (collapse removed nr8); dropdown-dismiss open-via-dispatch + outside-click. 3 code commits. compile clean; admin `mix test --seed 0` clean-DB 350/0. GOTCHA: the local e2e server seeds committed rows into the shared MIX_ENV=test DB → inflates `mix test` count assertions (executor's flagged webhooks_live_test:106 "failure" was this pollution, NOT real; `ecto.drop/create`→test_helper re-migrates→350/0). LESSON: `cd accrue_admin && mix test` runs NEITHER the Playwright UAT NOR the core package-docs gate — both CI-only; run `npx playwright test` + `verify_package_docs.sh` locally when touching admin UI | 2026-06-22 | 90952f5f | [260622-i2c-admin-uat-green-up](./quick/260622-i2c-admin-uat-green-up/) |

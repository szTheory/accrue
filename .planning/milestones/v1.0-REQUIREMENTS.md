...
Fresh requirements will be created by the next milestone via `-new-milestone`.
...
- [x] **FND-01**: `Accrue.Money` value type with zero-decimal and three-decimal currency safety, rejects bare integers at API boundaries
- [x] **FND-02**: `Accrue.Error` exception hierarchy (`APIError`, `CardError`, `RateLimitError`, `IdempotencyError`, `DecodeError`, `SignatureError`, `ConfigError`)
- [x] **FND-03**: `Accrue.Config` runtime config via NimbleOptions with auto-generated docs
- [x] **FND-04**: `Accrue.Telemetry` event naming conventions module (`[:accrue, :domain, :action, :start|:stop|:exception]`)
- [x] **FND-05**: `Accrue.Application` empty-supervisor pattern (host owns Repo/Oban/Finch lifecycle)
- [x] **FND-06**: Monorepo layout — sibling mix projects `accrue/` + `accrue_admin/`, independent Hex publishing
- [x] **FND-07**: Brand palette CSS variables (Ink/Slate/Fog/Paper + Moss/Cobalt/Amber)

### Processor

- [x] **PROC-01**: `Accrue.Processor` behaviour with callbacks for every Stripe operation Accrue needs
- [x] **PROC-02**: `Accrue.Processor.Stripe` first-party adapter delegating to lattice_stripe 0.3
- [x] **PROC-03**: `Accrue.Processor.Fake` in-memory ETS adapter — primary test surface (deterministic IDs, test clock, scriptable responses, event-trigger API)
- [x] **PROC-04**: Deterministic idempotency keys derived from `{operation, subject_id, canonical_params}` with retry-safe reuse
- [x] **PROC-05**: Stripe Connect context (Stripe-Account header) threaded through every processor call
- [x] **PROC-06**: Per-request API version override
- [x] **PROC-07**: Stripe error mapping to `Accrue.Error` hierarchy

### Billing Domain

- [x] **BILL-01**: Polymorphic `Accrue.Billing.Customer` with `owner_type`/`owner_id`, `data` jsonb, default-management, metadata deep-merge
- [x] **BILL-02**: `use Accrue.Billable` macro — any host schema (User, Organization, Team) becomes billable
...
- [x] **WH-01**: `Accrue.Webhook.Plug` raw-body capture **scoped to webhook routes only** (never global)
- [x] **WH-02**: Stripe signature verification with multi-secret rotation support
- [x] **WH-03**: DB idempotency via `accrue_webhook_events` with `UNIQUE(processor_event_id)`
- [x] **WH-04**: Oban-backed async dispatch with exponential backoff retry
- [x] **WH-05**: Dead-letter queue after N attempts (default 25)
- [x] **WH-06**: User handler behaviour with pattern-matchable event types
- [x] **WH-07**: Default handler for built-in state reconciliation (subscription/invoice/charge updates)
...
- [x] **WH-10**: Handler re-fetches current object instead of trusting snapshot payload
- [x] **WH-11**: Configurable DLQ retention, default 90 days, pruned via Oban cron
- [x] **WH-12**: Webhook pipeline p99 latency <100ms (verify → persist → enqueue → 200)
- [x] **WH-13**: Multi-endpoint webhook support with Connect-variant secret
- [x] **WH-14**: Webhook event type constants module (or dependency on lattice_stripe's)

### Event Ledger

- [x] **EVT-01**: `accrue_events` append-only table with Postgres role grants / trigger enforcing immutability (`REVOKE UPDATE/DELETE`)
- [x] **EVT-02**: Schema: id, type, schema_version, actor_type (NOT NULL enum), actor_id, subject_type, subject_id, data jsonb, trace_id, idempotency_key, inserted_at
- [x] **EVT-03**: `Accrue.Events.record/1` helper with Ecto.Multi support for transactional writes alongside state mutations
- [x] **EVT-04**: Every Billing context function that mutates state emits a corresponding event in the same transaction
...
- [x] **EVT-07**: OpenTelemetry `trace_id` correlation captured on every event write
- [x] **EVT-08**: Actor context enum: `user | system | webhook | oban | admin` with required actor_type
...
- [x] **MAIL-01**: `Accrue.Mailer` behaviour wrapping Swoosh (host configures Swoosh adapter)
...
- [x] **PDF-01**: `Accrue.PDF` behaviour with `render/2` callback
...
- [x] **AUTH-01**: `Accrue.Auth` behaviour (`current_user/1`, `require_admin_plug/0`, `user_schema/0`, `log_audit/2`, `actor_id/1`)
- [x] **AUTH-02**: `Accrue.Auth.Default` fallback adapter (dev-only; fails closed in prod)
...
- [x] **OBS-01**: `:telemetry` events for every public entry point with `:start`/`:stop`/`:exception`
...
- [x] **OBS-06**: Stripe error mapping to structured `Accrue.Error` with metadata preserved

### Testing

- [x] **TEST-01**: Fake Processor as primary test surface (Phase 1 foundation, not afterthought)
...
- [x] **TEST-09**: Oban.Testing integration for async assertions
...
- [x] **OSS-11**: MIT `LICENSE` file at monorepo root, per-package copies as needed

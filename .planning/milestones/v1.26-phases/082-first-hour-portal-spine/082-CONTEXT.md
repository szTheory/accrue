...
- **D-01:** Use **two parallel paragraphs** (checkout unchanged; **add** portal paragraph **immediately after** checkout) with **optional one-line intro** immediately above them, e.g. that both are server-side Stripe redirect helpers sharing the same documentation shape — **without** merging bodies into one paragraph or replacing with sparse bullets that drop arity, tuple literals, or anchors.

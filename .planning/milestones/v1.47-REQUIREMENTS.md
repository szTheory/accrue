# Requirements Archive: v1.47 ENT-10 Polish + Adopter-Proof Completeness
...
- [x] **PRF-03** — Developer adopting Accrue can verify their host app has all required Oban cron workers and queues wired by running `recovery_wiring_test.exs` — assertions cover all four cron workers (`DunningSweeper`, `DetectExpiringCards`, `MeterEventsReconciler`, `MeteredRenewalReconciler`) and all five required Oban queues (`accrue_webhooks`, `accrue_mailers`, `accrue_dunning`, `accrue_meters`, `accrue_scheduled`); `config.exs` includes append-merge comment for adopters with existing crontab

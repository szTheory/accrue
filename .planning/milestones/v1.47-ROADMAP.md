...
**Goal:** An adopter running `recovery_wiring_test.exs` gets a deterministic pass/fail signal that all four required Oban cron workers and all five required Oban queues are wired in their host config — with a `config.exs` comment showing the safe append-merge pattern for adopters who already have a crontab.
**Depends on:** Phase 153
**Requirements:** PRF-03
**Success Criteria** (what must be TRUE):
  ...
  2. `recovery_wiring_test.exs` asserts all five required Oban queues (`accrue_webhooks`, `accrue_mailers`, `accrue_dunning`, `accrue_meters`, `accrue_scheduled`) are declared in host config.

...
1. **Wave 0: RED interaction contracts.** Split browser contract/script, JS lifecycle unit contracts, and ExUnit source/theme/fixture/copy contracts into separate filesets so each contract plan remains small and executable. [VERIFIED: existing test patterns]
2. **Wave 1: Independent implementation starts.** Patch modal/drawer component/root structure, patch JS lifecycle hooks, and start copy-module helper work in parallel because the filesets do not overlap. [VERIFIED: overlay/client paths above; VERIFIED: copy paths above]
3. **Wave 2: Browser proof and copy call sites.** Prove overlay root/hit-testing after component and JS work; update list-page and detail/analytics call sites after copy helpers exist. [VERIFIED: Phase 195/198 browser patterns; VERIFIED: LiveView copy paths above]
4. **Waves 3-5: Sequential geometry/floating/theme split.** Run drawer geometry/reduced-motion, floating-panel bounds, and affordance/theme persistence as separate plans because they share CSS, generated CSS, and the Phase 199 browser spec. [VERIFIED: CSS/test paths above]
5. **Wave 6: Fixture stress.** Extend `E2E.Fixtures` only where existing seeds do not cover Phase 199 failure modes, then add route-level Playwright flows for FIX-01/FIX-02 using explicit target arrays. [VERIFIED: fixture/helper paths above]
6. **Wave 7: Action-context/export bridge.** Add hidden object/action context to shared components, update generated copy fixtures when browser copy tests depend on changed strings, and keep @copy assertions focused. [VERIFIED: `Detail.summary_list/1`; VERIFIED: `DropdownMenu.action_menu/1`; VERIFIED: export task path]
7. **Wave 8: Focused Phase 199 closeout.** Run focused ExUnit/JS/Playwright gates, rebuild committed admin assets if CSS/JS changed, run copy export if copy fixtures depend on changed strings, then run relevant package-doc/guardrail checks. [VERIFIED: `accrue_admin/lib/mix/tasks/accrue_admin.assets.build.ex`; VERIFIED: `scripts/ci/verify_package_docs.sh`; VERIFIED: `accrue_admin/package.json`]
...
1. **Command palette routing:** RESOLVED: Phase 199 will first test the command palette as a named wrapper with overlay-equivalent focus, dismissal, hit-test, backdrop, bounds, and cleanup behavior. Executors refactor it through `Overlay.overlay/1` only if those focused tests cannot be satisfied by the existing `GlobalSearch` / `CommandPalette` ownership model. This follows D-01, D-08, and the verified current command-palette paths.
2. **Popover shared helper threshold:** RESOLVED: Phase 199 will keep positioning dependency-light. Executors add only small local helper code when repeated near-edge assertions across dropdown, command palette, theme picker, or tooltip/popover surfaces show duplicated or inconsistent geometry; no package-manager dependency is planned. This follows D-08 and D-22.
3. **Fixture extension scope:** RESOLVED: Phase 199 will reuse `/__e2e__/seed/phase191-matrix`, `/operator-flows`, `/edge-states`, and `/overflow` first. A narrow `seed_phase199_interactions!/0` and `/__e2e__/seed/phase199-interactions` endpoint may be added only for missing route IDs or edge values required by D-13/D-14, with deterministic IDs and idempotency tests.
4. **Exact copy wording:** RESOLVED: Exact strings are executor discretion bounded by `brandbook/voice.md`, `brandbook/copy.md`, D-16 through D-19, and CPY-01 tests. New or touched page-level copy routes through `AccrueAdmin.Copy` or domain copy modules, and generated copy fixtures are regenerated when browser tests consume changed strings.

...
- [x] **Phase 207: Orchestration + digest + one-command round/fix loop** - Two `mix accrue_admin.ui.*` commands drive the whole pipeline with a rendered digest, minimal batch-approve checkpoints, auto-minted guards, and guaranteed termination, plus proposer prompt-caching + a surface-subset filter (ORCH-01..08) (completed 2026-07-07)

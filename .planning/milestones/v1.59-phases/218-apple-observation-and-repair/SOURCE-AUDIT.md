...
| REQ | AAPL-03 | Idempotent convergence and repairable quarantine | 01, 04, 07 | COVERED | Durable intake and concrete reconciliation-wakeup outbox, notification acknowledgement, closed dispositions, and permutation/race proof. |
| REQ | AAPL-04 | Scheduled status/history reconciliation and lifecycle bounds | 01, 05, 06 | COVERED | Wave-1 durable wakeup producer, later host-owned Oban drain/final-page checkpoint, and source-local lifecycle ordering. |
...
| RESEARCH | — | Transactional durable reconciliation wakeup before any producer | 01, 04, 05, 07 | COVERED | Plan 01 creates the named unique outbox and `enqueue_in_transaction/5`; bind, repair, and notification call it; Plan 05 drains it into host-owned Oban work. |
...
| CONTEXT | D-02 | Transactional verified-token claim and projection | 01, 04 | COVERED | `Projector.project_in_transaction/3` plus `ReconciliationWakeup.enqueue_in_transaction/5` keep bind, Observation, grant/revision, audits, Projector follow-up job, and the named unique wakeup row in one rollback-tested transaction. |
...
| CONTEXT | D-12 | Coalesced host-owned Oban repair | 01, 04, 05, 07 | COVERED | Wave-1 transactional wakeup producer is used by every early producer; Plan 05 claims/drains it into scalar-argument Oban work with bounded concurrency/backoff/rate budget. |

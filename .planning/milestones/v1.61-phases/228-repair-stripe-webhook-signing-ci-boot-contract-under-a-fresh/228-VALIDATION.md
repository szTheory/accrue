...
## Phase 228 Terminal Automation Coverage

- The exact attempt-1 `workflow_dispatch` run is bound to immutable repository, SHA, time-window, job, step, and artifact facts by the live-binding verifier.
- Terminal reconciliation accepts the sanitized `failed/selected_assertions_failed` tuple: the boot preflight succeeded, ten selected assertions ran, five passed, five failed, and no selected assertion was skipped.
- Every completion command is executable and machine judged; `human_judgment:false` applies to both terminal controls. The generated UAT is permitted only after a passed phase verification with `behavior_unverified: 0`.

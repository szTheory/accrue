...
- **D-43:** Add `"group-pull-request-title-pattern": "chore: release accrue-monorepo ${version}"`. A grouped PR title that cannot yield a version silently skips GitHub Release and tag creation (open upstream issues #2306/#2712); current titles carry no `${version}`. Latent, not yet biting.
  - **SUPERSEDED 2026-09-18 — this decision caused the failure it was meant to prevent.** release-please's Merge plugin sources the grouped title's `${version}` from the release candidate whose path is exactly `.`. This repo has no package at `.`, so the version is `undefined` and `${version}` rendered EMPTY, producing `chore: release accrue-monorepo`. That title then failed `PullRequestTitle.parse` at release time, so release `1.6.0` (PR #46) created **zero tags and zero GitHub Releases while the workflow reported `success`**, and would have blocked every later release PR. Reverted; the key is now absent and guarded by `scripts/ci/verify_release_pr_title_roundtrip.sh`. Adding the key is only safe once a root package at `.` exists. See `RELEASING.md`.

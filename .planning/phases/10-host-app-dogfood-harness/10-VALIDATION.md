...
| 10-07-02 | 07 | 6 | HOST-07 | T-10-18 | Admin must inspect seeded or flow-created billing state in `/billing`, including a customer/subscription identifier, `price_basic` or equivalent subscription state like `active`, a webhook row/status, and event history/activity text, before replay; replay/requeue then produces persisted `admin.webhook.replay.completed` audit/event evidence through the mounted UI | live/integration | `cd examples/accrue_host && mix test test/accrue_host_web/admin_webhook_replay_test.exs` | ❌ W0 | ⬜ pending |

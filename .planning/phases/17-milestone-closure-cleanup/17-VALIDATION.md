...
| **Quick run command** | `cd accrue && mix test test/accrue/docs/release_guidance_test.exs test/accrue/docs/package_docs_verifier_test.exs --trace` |
...
- **After browser seed cleanup task commit:** Run `bash scripts/ci/accrue_host_uat.sh`
- **After every plan wave:** Run both quick docs contracts and `bash scripts/ci/accrue_host_uat.sh`
...
- **Max feedback latency:** 120 seconds for docs-only feedback; host UAT may exceed this but is required for fixture cleanup confidence
...
| 17-01-02 | 01 | 1 | Audit cleanup | T-17-03 | Seed cleanup deletes only fixture-owned webhook/payment-failed/replay rows and preserves unrelated shared DB history | host integration | `bash scripts/ci/accrue_host_uat.sh` | yes | pending |

...
| 214-01-03 | 01 | 1 | DOCS-03 | T-214-02 | Active planning mirrors reconcile without rewriting dated evidence | scoped planning assertions | `bash -ceu "$(perl -0777 -ne 'if (/<name>Task 3: Reconcile active planning status.*?<automated>(.*?)<\/automated>/s) { $x=$1; $x =~ s/&amp;/&/g; $x =~ s/&lt;/</g; $x =~ s/&gt;/>/g; print $x }' .planning/phases/214-docs-truth-reconciliation/214-01-PLAN.md)"` | ✅ existing | ⬜ pending |

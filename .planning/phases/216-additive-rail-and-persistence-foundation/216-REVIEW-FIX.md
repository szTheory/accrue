...
**Commit:** 2041b197
**Applied fix:** Replaced the substring deny-list with the Phase-216 fixed metadata projection contract (empty metadata or allowlisted `source` values), enforced it with a PostgreSQL JSONB check, and covered neutral-key PII and payload-fragment bypasses.
...
**Commit:** 1028bf3e
**Applied fix:** Constrained installation and thumbprint values to bounded opaque tokens without introducing a proof-verification format, added matching PostgreSQL checks and direct-write tests, and retained account-scoped uniqueness/history semantics.
...
`mix format --check-formatted` and `git diff --check` passed for every fix. The focused suites and `mix test --warnings-as-errors` were attempted but could not start because the isolated worktree lacks fetched Mix dependencies; Mix reported missing dependencies before compilation.

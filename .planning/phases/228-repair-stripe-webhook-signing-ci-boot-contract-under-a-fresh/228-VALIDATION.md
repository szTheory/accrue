...
- [ ] Add the Phase 228 bypass-impossibility fixture: reject `skipped` / `intentional_bypass`, assert `run_live_stripe` is the sole dispatch input for this path, prove the manual job is input-gated/executed, and prove the finalizer passes no bypass flag or reason.

...
| .tool-versions | committed | Re-verified 2026-09-16: \`.tool-versions\` is already git-tracked on \`integration/v1.62-candidate\` (committed by 230-05: nodejs 22.14.0, elixir 1.19.5-otp-28, erlang 28.5) and will become tracked on this branch once the 232 re-cut merges that line in. This REVERSES the prior never-git-tracked intent recorded elsewhere in the repository's history; recorded explicitly here (D-50) rather than letting it flip silently. Needs maintainer confirmation of the reversal. | — | integration/v1.62-candidate:.tool-versions | — |
| scripts/ci/stripe_test_fixtures.mjs | committed | Re-verified 2026-09-16: not orphaned work. Tracked \`scripts/ci/provider_proof_automation.mjs\`'s RELEVANT_PATHS allowlist (line 15) names this exact filename in its scope regex, and \`.github/workflows/ci.yml\` invokes \`provider_proof_automation.mjs\` (lines 62, 1641) as part of the provider-proof automation, which references this script by name. The repository ships CI code referencing a file present in no tree; disposition is to commit it, not remove it. | — | scripts/ci/provider_proof_automation.mjs (RELEVANT_PATHS allowlist) | — |
| scripts/ci/verify_stripe_test_fixtures.mjs | committed | Re-verified 2026-09-16: same allowlist entry and CI invocation path as scripts/ci/stripe_test_fixtures.mjs -- provider_proof_automation.mjs's RELEVANT_PATHS regex (line 15) names this filename explicitly. Disposition is to commit it, not remove it. | — | scripts/ci/provider_proof_automation.mjs (RELEVANT_PATHS allowlist) | — |

...
- **Now:** `node --version` → `v24.19.0` (`which node` → `/Users/jon/.asdf/shims/node`; the repo's own `.tool-versions` — tracked on the candidate branch, untracked on HEAD per D-50 — does not pin a `nodejs` entry, only `erlang`/`elixir`). Empirically probed via a script at a space-containing path:

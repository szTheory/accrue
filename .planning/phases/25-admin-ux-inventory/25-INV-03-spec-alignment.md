...
| Webhooks | `20-UI-SPEC.md` — Webhook detail + ambiguous ownership | Aligned | C-04, C-05; **26-03** (`webhooks_live.ex` / `webhook_live.ex` typography parity); **27-03** (`Copy` / `Copy.Locked` + webhooks index Tier A empty state) |
...
| C-04 | `20-UI-SPEC.md` — `### Webhook detail and bulk replay` | Aligned | `admin`, `webhooks` | `accrue_admin/lib/accrue_admin/live/webhook_live.ex` (2026-04-20 **26-03**: nested `ax-page` removed from replay confirm + forensic stack); `accrue_admin/test/accrue_admin/live/webhook_live_test.exs`, `accrue_admin/test/accrue_admin/live/webhook_replay_test.exs`; `examples/accrue_host/e2e/phase13-canonical-demo.spec.js`; **2026-04-20 Phase 27 plan `27-03`:** `copy.ex`, `copy/locked.ex`, `webhook_live.ex`, `webhooks_live.ex`, `webhook_live_test.exs`, `webhooks_live_test.exs` | maintainer | — |
| C-05 | `20-UI-SPEC.md` — `### Ambiguous ownership behavior` | Aligned | `admin`, `webhooks` | `accrue_admin/lib/accrue_admin/live/webhook_live.ex`; `accrue_admin/test/accrue_admin/live/webhook_live_test.exs` (**26-03** `ax-kpi-grid` / single-`ax-page` checks); **2026-04-20 Phase 27 plan `27-03`:** ambiguous replay copy in `AccrueAdmin.Copy.Locked` + `webhook_live.ex` | maintainer | — |

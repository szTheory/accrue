...
| Entitlements / plan-gating (`has_active_plan?`, plugs/guards) | ✅ | `Accrue.entitled?/2`, `has_active_plan?/2`, `features_for/1`, `entitlement_quantity/2`; `Accrue.Plug.RequireEntitlement` + `require_feature`/`require_plan` macros; `Accrue.Live.Entitlements` `on_mount`; admin entitlements tab | v1.39. Local-identical across providers; `entitling?/1` lifecycle truth + `past_due_grace` knob. Optional Stripe-native sync shipped as an off-by-default observational advisory cache for diagnostics/admin reads; it never gates. Was SEED-002 #4. |

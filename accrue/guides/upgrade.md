...
**Repository history vs packages:** repository-internal docs may use
maintainer release labels that do not match the Hex major line.
Consumers should treat the published package version as the upgrade
boundary. `accrue` and `accrue_admin` publish in lockstep on the
**`1.0.x`** line. For the 1.0.0 bootstrap story (historical reference
for the cut event), read
**[`RELEASING.md`](https://github.com/szTheory/accrue/blob/main/RELEASING.md)**
→ *Appendix: Same-day `1.0.0` bootstrap*.

defmodule Accrue.Emails.InvoiceFinalized do
  ...
  Mailglass-backed notification for finalized invoices.
  """
  ...
  def subject(assigns) when is_map(assigns) do
    ...
    invoice = invoice(assigns)

    cond do
      map_get(invoice, :number) && map_get(branding, :business_name) ->
        "Invoice #{map_get(invoice, :number)} from #{map_get(branding, :business_name)}"

      map_get(branding, :business_name) ->
        "Your invoice from #{map_get(branding, :business_name)}"
  ...
  def message(assigns) when is_map(assigns) do
    ...
    |> Mailglass.Message.put_function(:invoice_finalized)
  end
    ...
    <Mailglass.Components.Layout.email_layout title={@subject}>
      <.container>
        ...
        <.section padding="0 24px">
          <Accrue.Invoices.Components.invoice_header context={@context} />
        </.section>
        ...
        <.section padding="0 24px">
          ...
          <.text>
            Hi {@customer.name || "there"}, your invoice for <strong>{@formatted_total}</strong>
            is now available. A PDF copy is attached. You can also view it online:
            <a href={@hosted_invoice_url}>{@hosted_invoice_url}</a>.
          </.text>
        </.section>

        <.section padding="0 24px">
          <Accrue.Invoices.Components.line_items context={@context} />
          <Accrue.Invoices.Components.totals context={@context} />
        </.section>
  ...
  defp template_assigns(assigns) do
    ...
    invoice = normalize_map(invoice(assigns))
  ...
  defp invoice(assigns) do
    context = context(assigns)
    map_get(context, :invoice) || map_get(assigns, :invoice) || %{}
  end

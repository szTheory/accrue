defmodule Accrue.Emails.InvoicePaid do
  ...
  Mailglass-backed confirmation for paid invoices.
  """
  ...
  def subject(assigns) when is_map(assigns) do
    ...
    cond do
      map_get(invoice, :number) ->
        "Payment received for invoice #{map_get(invoice, :number)}"

      map_get(branding, :business_name) ->
        "Payment received at #{map_get(branding, :business_name)}"
  ...
  def message(assigns) when is_map(assigns) do
    ...
    |> Mailglass.Message.put_function(:invoice_paid)
  end
    ...
    <Mailglass.Components.Layout.email_layout title={@subject}>
      <.container>
        ...
        <.section padding="0 24px">
          ...
          <.text>
            Thanks {@customer.name || "there"} — we received your payment of
            <strong>{@formatted_total}</strong> for invoice <strong>{@invoice.number}</strong>.
          </.text>
        </.section>

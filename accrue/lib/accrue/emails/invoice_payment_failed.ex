defmodule Accrue.Emails.InvoicePaymentFailed do
  ...
  Mailglass-backed reminder for failed invoice payments.
  """
  ...
  def subject(assigns) when is_map(assigns) do
    ...
    case map_get(invoice, :number) do
      nil -> "Action required: invoice payment failed"
      number -> "Action required: payment failed for invoice #{number}"
    end
  end
  ...
  def message(assigns) when is_map(assigns) do
    ...
    |> Mailglass.Message.put_function(:invoice_payment_failed)
  end
    ...
    <Mailglass.Components.Layout.email_layout title={@subject}>
      <.container>
        ...
        <.section padding="0 24px">
          ...
          <.text>
            Hi {@customer.name || "there"}, your payment of <strong>{@formatted_total}</strong>
            for invoice <strong>{@invoice.number}</strong> could not be processed. Pay now to keep your account current.
          </.text>

          <.button :if={@hosted_invoice_url} href={@hosted_invoice_url}>Pay now</.button>

          <.text :if={@hosted_invoice_url}>

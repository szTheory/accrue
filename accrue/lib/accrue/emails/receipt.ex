defmodule Accrue.Emails.Receipt do
  ...
  def subject(assigns) when is_map(assigns) do
    ...
    case map_get(branding, :business_name) do
      nil -> "Receipt"
      business_name -> "Receipt from #{business_name}"
    end
  end
    ...
    <Mailglass.Components.Layout.email_layout title={@subject}>
      <.container>
        ...
        <.section padding="0 24px">
          ...
          <.text>
            Hi {@customer.name || "there"}, we received your payment of
            <strong>{@formatted_total}</strong> for invoice
            <strong>{@invoice.number || @charge_id || "unknown"}</strong>.
          </.text>
        </.section>
  ...
  defp template_assigns(assigns) do
    ...
    %{
      ...
      preview: map_get(assigns, :preview) || "Thanks for your payment",

defmodule Accrue.Emails.SubscriptionResumed do
  ...
  Mailglass-backed notification when a paused subscription resumes.
  """
  ...
  def subject(assigns) when is_map(assigns) do
    ...
    case map_get(branding, :business_name) do
      nil -> "Your subscription has resumed"
      business_name -> "Your #{business_name} subscription has resumed"
    end
  end
  ...
  def message(assigns) when is_map(assigns) do
    ...
    |> Mailglass.Message.put_function(:subscription_resumed)
  end
    ...
    <Mailglass.Components.Layout.email_layout title={@subject}>
      <.container>
        ...
        <.section padding="0 24px">
          <.heading level={1}>Your subscription has resumed</.heading>

          <.text>
            Hi {@customer.name || "there"}, your {@branding.business_name} subscription has resumed.
            Normal billing will continue on the next cycle.
          </.text>
        </.section>

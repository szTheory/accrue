defmodule Accrue.Emails.TrialEnded do
  ...
  Mailglass-backed notification for trials that have ended.
  """
  ...
  def subject(assigns) when is_map(assigns) do
    ...
    case map_get(branding, :business_name) do
      nil -> "Your trial has ended"
      business_name -> "Your #{business_name} trial has ended"
    end
  end
  ...
  def message(assigns) when is_map(assigns) do
    ...
    |> Mailglass.Message.put_function(:trial_ended)
  end
    ...
    <Mailglass.Components.Layout.email_layout title={@subject}>
      <.container>
        ...
        <.section padding="0 24px">
          <.heading level={1}>Your trial has ended</.heading>

          <.text>
            Hi {@customer.name || "there"}, your {@branding.business_name} trial has ended.
            To continue, add a payment method to activate your subscription.
          </.text>
  ...
  defp template_assigns(assigns) do
    ...
    %{
      context:
        ...
        |> Map.put_new(:cta_url, map_get(context, :cta_url) || map_get(assigns, :cta_url))

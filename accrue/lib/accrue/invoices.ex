defmodule Accrue.Invoices do
  ...
  ## Contract
    ...
    * `render_invoice_pdf/2` — returns `{:ok, binary}` on success,
      ...
      adapter is `Accrue.InvoiceRenderer.Null`, and
      `{:error, %Accrue.Error.InvoiceRendererUnavailable{}}` when the
      configured adapter is `Accrue.InvoiceRenderer.ChromicPDF` but the
  ...
  The `v1.0` design intentionally never persists PDF bytes because:
    ...
    3. The default Rendro renderer keeps the install footprint cleaner by
       avoiding a Chrome dependency for the common invoice path.
  ...
  ## Options
    ...
    * `:size`, `:paper_width`, `:paper_height`, `:margin_top`,
      ...
      to the optional Chromic renderer path

  ## Return values

    * `{:ok, binary}` — rendered PDF body
    * `{:error, %Accrue.Error.PdfDisabled{}}` — adapter is
      `Accrue.InvoiceRenderer.Null`; caller should fall through to the Stripe
      `hosted_invoice_url` link instead of attaching a PDF
    * `{:error, %Accrue.Error.InvoiceRendererUnavailable{}}` — adapter is
      `Accrue.InvoiceRenderer.ChromicPDF` but the host app has not started
  ...
  # host app has `Accrue.InvoiceRenderer.ChromicPDF` configured but forgot to add
    ...
    if adapter == Accrue.InvoiceRenderer.ChromicPDF do
      if Process.whereis(ChromicPDF) do
        ...
        Accrue.Telemetry.Ops.emit(
          :pdf_adapter_unavailable,
          %{count: 1},
          %{adapter: :chromic_pdf, surface: :invoice_pdf}
        )
        ...
        {:error,
         %Accrue.Error.InvoiceRendererUnavailable{
           ...
           message:
             "invoice_pdf_adapter is Accrue.InvoiceRenderer.ChromicPDF but the ChromicPDF process is not running"
         }}
      end
    else
      :ok

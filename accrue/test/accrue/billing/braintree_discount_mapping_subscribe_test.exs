defmodule Accrue.Billing.BraintreeDiscountMappingSubscribeTest do
  ...
  defmodule SubscriptionGatewayStub do
    def create(params, _opts) do
      ...
      if Process.get(:gateway_failure_reason) do
        send(self(), {:gateway_redemption_count_before_failure, current_redemption_count()})
        {:error, Process.get(:gateway_failure_reason)}
      else
        send(self(), {:gateway_redemption_count, current_redemption_count()})
  ...
  describe "subscribe/3 with promotion_code" do
    ...
    test "releases a reserved redemption when the gateway create fails" do
      customer = insert_braintree_customer!()
      ...
      Process.put(:gateway_failure_reason, :gateway_timeout)

defmodule Accrue.Entitlements.FakeFixtureTest do
  ...
  test "record fixtures cover every supported rail and environment with bounded fake evidence" do
    ...
    assert Enum.map(pairs, fn pair ->
             ...
             {{observation.rail, observation.environment}, {grant.rail, grant.environment},
              device.state}
           end) ==
             Enum.map(pairs, fn {rail, environment} ->
               {{rail, environment}, {rail, environment}, :active}
             end)
  ...
  test "named scenarios are sorted, unique, and exercise real persistence constraints" do
    ...
    assert {:ok, account} =
             Account.fetch_or_create(TestRepo, account_attrs.owner_type, account_attrs.owner_id)
    ...
    assert {:ok, _} =
             TestRepo.update(
               Grant.changeset(first_grant, %{superseded_at: ~U[2026-08-02 16:00:00.000000Z]})
             )
  ...
  test "fixture calls and scenario values remain stable without provider dependencies" do
    ...
    assert EntitlementFixtures.account_attrs("stable") ==
             EntitlementFixtures.account_attrs("stable")

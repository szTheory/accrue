defmodule Accrue.Entitlements.RepairDrillsTest do
  ...
  test "named reconciliation repairs are authorized, bounded, audited, and idempotent", %{
    account: account
  } do
    ...
    assert 1 ==
             TestRepo.aggregate(
               from(event in Event,
                 where:
                   event.subject_id == ^account.id and
                     event.type == "entitlements.repair.retry_missed_notification"
               ),
               :count
             )
  end

  test "dry runs and review-only incidents do not mutate ownership or finance", %{
    account: account
  } do
  ...
  test "outage, device, key, and backlog repairs retain bounded outcomes and converge through diagnostics",
       %{account: account} do
    for {action, target, operation_id} <- [
          {:retry_provider_check, %{lineage_id: "lineage-220", environment: :production},
           "repair-220-outage"},
  ...
  test "an unauthorized or malformed request cannot reach the effect callback", %{
    account: account
  } do
    effect = fn _target ->
      send(self(), :effect_called)
      {:ok, :queued}
    end
    ...
    assert {:error, :invalid_identifier} =
             Repair.retry_missed_notification(
               account,
               %{lineage_id: "", environment: :production},
               repair_opts("repair-220-invalid", effect: effect)
             )

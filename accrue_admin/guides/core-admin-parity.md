...
| `/customers/:id` | `AccrueAdmin.Live.CustomerLive` `:show` | **gaps** — shell headings, KPI labels, tab titles, and several empty states remain raw English; flashes and selected empty rows use `Copy` / `Copy.Locked`. **ADM-13** **`payment_methods`** tab operator inventory SSOT: `.planning/phases/76-customer-pm-tab-inventory-copy-burn-down/76-VERIFICATION.md` (aligns ADM-13/14 scope without duplicating the ADM-07 matrix). | **clean** on `ax-*` cards and lists | — | planned — Phase 55 (ADM-09) | P1 |

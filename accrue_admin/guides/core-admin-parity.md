...
| `/invoices` | `AccrueAdmin.Live.InvoicesLive` `:index` | **gaps — P0 (ADM-08)** — list header, KPI labels, breadcrumbs, filters, and column labels are raw English; only empty-state slots use `Copy` today | **clean** on `ax-*` shell; **needs audit** for dense table chrome | — | planned — Phase 55 (ADM-09) | P0 |
| `/invoices/:id` | `AccrueAdmin.Live.InvoiceLive` `:show` | **gaps — P0 (ADM-08)** — invoice shell, KPIs, workflow buttons, tax-risk copy, and section titles are raw English; flashes and select warnings use `Copy` | **clean** on `ax-*` shell; **needs audit** for long-form risk copy | — | planned — Phase 55 (ADM-09) | P0 |
...
The **VERIFY-01 lane** column records how the **existing** host VERIFY spine relates to each row today. Most core rows remain **`planned — Phase 55 (ADM-09)`** until named flows and merge-blocking coverage land—this preserves an honest inventory without silently expanding VERIFY policy (**D-20**, **D-14**).

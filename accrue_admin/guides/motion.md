...
| 3 | **More overflow** (`.ax-tab-more-menu` in `customer_live.ex`) | toggle open | identical to dropdown_menu | same as dropdown_menu | same as dropdown_menu | same as dropdown_menu | **Affordance** — recessed Customer-360 tabs reveal with the same grammar as any dropdown (consistency). |
| 4 | **collapsible nav group** (`sidebar.ex` + `sidebar_collapse.js`, `.ax-sidebar-nav-group`) | toggle (`aria-expanded`; JS sets `list.hidden`) | `--ax-sidebar-group-chevron` rotate (already wired at `app.css:1218`) + group reveal | chevron rotate via `--ax-transition-transform`; reveal: `opacity --ax-dur-2 --ax-ease-out` on the controlled list | symmetric (collapse: `opacity --ax-dur-exit --ax-ease-in`) | chevron rotate→instant, opacity retained, dur→instant | **Feedback** — chevron confirms the toggle registered; reveal shows where the items went. Animate `opacity`, not `height`/`auto` (layout thrash — antipattern A2). The `hidden` attr stays the structural toggle. |

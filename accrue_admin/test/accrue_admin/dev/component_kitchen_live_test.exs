defmodule AccrueAdmin.ComponentKitchenLiveTest do
  ...
  test "renders the shared component kitchen and floating toolbar", %{conn: conn} do
    ...
    assert html =~ "Sandbox examples only"
    assert html =~ "Primary action"
    assert html =~ "Secondary action"
    assert html =~ "Demo queued invoice jobs"
    ...
    assert html =~ "Demo billing status: unhealthy"
    assert html =~ "Sample webhook delivery failures"
    assert html =~ "Demo only. Use Dashboard for production billing health."
    assert html =~ "Demo queue unhealthy: 7 jobs"
    assert html =~ "Target is 0 queued jobs"

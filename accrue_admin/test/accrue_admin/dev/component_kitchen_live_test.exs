defmodule AccrueAdmin.ComponentKitchenLiveTest do
  ...
  test "renders the shared component kitchen and floating toolbar", %{conn: conn} do
    ...
    assert html =~ "Dev-only specimens"
    ...
    assert html =~ "Billing unhealthy: webhook delivery failures"
    assert html =~ "Fix failed/dead webhook deliveries before billing is healthy"
    assert html =~ "Open failed-webhook debugger to restore overall billing health"
    assert html =~ "Unhealthy above 0-job target"
    assert html =~ "Healthy target: 0 queued invoice jobs"

defmodule AccrueAdmin.ComponentKitchenLiveTest do
  ...
  test "renders the shared component kitchen and floating toolbar", %{conn: conn} do
    ...
    assert html =~ "Customer detail is the primary support path"
    ...
    assert html =~ "Primary action: find one customer record"
    ...
    assert html =~ "Open invoice queue view: 2 open invoices"

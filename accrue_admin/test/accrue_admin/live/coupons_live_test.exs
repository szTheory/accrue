defmodule AccrueAdmin.CouponsLiveTest do
  ...
  test "renders Coupons through PageHeader with the Phase 197 LIST chrome", %{conn: conn} do
    contract = ListContracts.fetch!(:coupons)
    conn = Phoenix.ConnTest.init_test_session(conn, admin_token: "admin")
    ...
    assert html =~ "Check which coupon definitions can still be applied."
    assert_single_filter_form(html)
  end

  test "bare coupons route represents the Valid coupons default lens", %{conn: conn} do
    contract = ListContracts.fetch!(:coupons)
    conn = Phoenix.ConnTest.init_test_session(conn, admin_token: "admin")
  ...
  test "coupon clear-all routes to All while preserving organization scope", %{conn: conn} do
    ...
    assert {:ok, _view, html} =
             live(conn, "/billing/coupons?org=allowed-org&valid=true&q=Spring")

    assert html =~ ~s(data-ax-clear-all)
    assert html =~ ~s(href="/billing/coupons?org=allowed-org&amp;view=all")
    refute html =~ "valid=true"
    refute html =~ "q=Spring"
  end

  test "distinguishes coupon populated, first-run-empty, filtered-empty, queue-empty, and loading states",
       %{conn: conn} do
    contract = ListContracts.fetch!(:coupons)
    ...
    assert populated_html =~ "Spring 25"
    ...
    insert_coupon(%{
      name: "Invalid only",
      processor_id: "coupon_invalid_only",
      valid: false
    })
    ...
    assert {:ok, _view, queue_html} = live(queue_conn, contract.route <> "?valid=true")
    ...
    assert {:ok, _view, filtered_html} =
             live(filtered_conn, contract.route <> "?view=all&q=___phase197_no_coupon___")
  ...
  test "prioritizes coupon identity, state, value, time, and signals before raw ids", %{
    conn: conn
  } do
    conn = Phoenix.ConnTest.init_test_session(conn, admin_token: "admin")

    assert {:ok, _view, html} = live(conn, "/billing/coupons?view=all")
    ...
    assert_before(html, "Spring 25", "coupon_spring")
  end

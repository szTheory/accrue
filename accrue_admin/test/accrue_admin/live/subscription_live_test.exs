defmodule AccrueAdmin.SubscriptionLiveTest do
  ...
  test "renders the six-band detail structure with one related resources strip", %{
    ...
    assert html =~ "Inspect failed delivery payload and retry trail"
    assert html =~ "Open this subscription&#39;s invoice context"
    assert html =~ "Queue workspace"
    assert html =~ "Open local invoice context"
    assert html =~ "Primary invoice queue workspace"
    assert html =~ "Open global dunning funnel"
  ...
  test "summary and drill bands replace page-level KPI and predicate noise", %{
    ...
    assert html =~ "Open local invoice context"
    assert html =~ "Open this subscription&#39;s invoice context"
    assert html =~ "payload, response, retry trail, and replay controls"
    assert html =~ "No subscription-level campaign"
    assert html =~ "Global recovery analytics still shows at-risk accounts and funnel trends."
    assert html =~ "Work open-invoice queue now:"
    assert html =~ "Audit event table: Actor / Action / Timestamp"

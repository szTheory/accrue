defmodule AccrueAdmin.SubscriptionLiveTest do
  ...
  test "renders the six-band detail structure with one related resources strip", %{
    ...
    assert html =~ "failed or dead delivery"
    assert html =~ "Debug all failed webhooks end-to-end"
  ...
  test "summary and drill bands replace page-level KPI and predicate noise", %{
    ...
    assert html =~ "Billing healthy? Yes - revenue can flow"
    ...
    assert html =~ "Open local invoice context"
    assert html =~ "Open this subscription&#39;s invoice context"
    assert html =~ "Open payload, response, retry history, and replay controls"

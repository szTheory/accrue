defmodule AccrueAdmin.SubscriptionLiveTest do
  ...
  test "renders the six-band detail structure with one related resources strip", %{
    ...
    assert html =~ "Primary: open dunning funnel and at-risk workspace"
  ...
  test "summary and drill bands replace page-level KPI and predicate noise", %{
    ...
    assert html =~ "Billing unhealthy: renewal, price, or amount is missing"
    assert html =~ "Fix missing billing setup, then confirm renewal and charges here."
    refute html =~ "Billing health right now"
    assert html =~ "MRR"
    assert html =~ "Missing amount"

defmodule AccrueAdmin.SubscriptionLiveTest do
  ...
  test "renders the six-band detail structure with one related resources strip", %{
    ...
    assert html =~ "Open filtered invoice queue for this subscription"
    assert html =~ "Watch dunning funnel + at-risk accounts"
    assert html =~ "Invoice queue first"
  ...
  test "summary and drill bands replace page-level KPI and predicate noise", %{
    ...
    assert html =~ "Setup is incomplete, so charges cannot be trusted"

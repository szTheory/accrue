defmodule AccrueAdmin.SubscriptionLiveTest do
  ...
  test "renders the six-band detail structure with one related resources strip", %{
    ...
    assert html =~ "Primary debugger for failed/dead deliveries, retries, and event links"
    assert html =~ "Debug failures"
  ...
  test "summary and drill bands replace page-level KPI and predicate noise", %{
    ...
    assert html =~ "Needs sync check"
    assert html =~ "Active billing has local projection gaps"
    assert html =~ "MRR"
    assert html =~ "Amount not protected locally"
    assert html =~ "verify processor sync"

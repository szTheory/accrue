...
build_run() {
  ...
  jq -e '(.id|type=="number") and (.name|type=="string") and (.event|type=="string") and (.head_branch|type=="string" and length>0) and (.head_sha|type=="string") and (.run_attempt|type=="number") and (.created_at|type=="string") and (.updated_at|type=="string") and (.html_url|type=="string")' "$run" >/dev/null || fail "malformed run response"
  ...
  jq -e --argjson policy "$(cat "$policy_manifest")" --argjson jobs "$(cat "$jobs")" --argjson artifacts "$(cat "$artifacts")" '
    ...
    . as $raw | {id:$raw.id,workflow:$raw.name,event:$raw.event,head_branch:$raw.head_branch,ref:("refs/heads/" + $raw.head_branch),head_sha:$raw.head_sha,attempt:$raw.run_attempt,status:$raw.status,conclusion:$raw.conclusion,created_at:$raw.created_at,completed_at:$raw.updated_at,url:$raw.html_url} as $base |
    ...
    ($base.event=="workflow_dispatch" and $base.head_branch=="main" and $base.ref=="refs/heads/main" and $base.attempt==1 and $base.status=="completed" and $base.conclusion=="success") as $base_eligible |

...
(
  ...
  ASDF_ERLANG_VERSION=28.0 ASDF_ELIXIR_VERSION=1.19.5-otp-28 bash -c 'cd accrue && mix format --check-formatted'
  ASDF_ERLANG_VERSION=28.0 ASDF_ELIXIR_VERSION=1.19.5-otp-28 bash -c 'cd accrue && mix test test/accrue/backend_automation_contract_test.exs --warnings-as-errors'
)

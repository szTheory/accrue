...
export function deriveStagedPathPercentiles(records, validationContext) {
...
export function renderBaseline(records, validationContext) {
  ...
  let staged = null;
  try { staged = deriveStagedPathPercentiles(records, validationContext); } catch { /* Rendering remains useful for incomplete evidence; the verifier is fail-closed. */ }
  const stagedConclusion = staged?.conclusion || "insufficient_sample — no critical-path percentile claim";
  // D-17 (231-04): a single-event-class evidence set (every "run" record shares
  // one event_class) is a targeted proof, not a mixed cohort -- surface its
  // class in the very first section so a workflow_dispatch-class proof can
  // never be silently read as a pull-request-class proof. A mixed cohort (like
  // Phase 226's 90-day baseline, which spans push/pull_request/workflow_dispatch/
  // schedule runs) never triggers this note, so that pre-existing evidence's
  // rendered output is unchanged.
  const runEventClasses = [...new Set(runRows.map((run) => run.event_class))];
  const eventClassNote = runEventClasses.length === 1 && runEventClasses[0] === "workflow_dispatch"
    ? " This is a `workflow_dispatch`-class proof; a future pull-request-class proof for the same SHA produces a second, distinct check-run set (see Phase 232)."
    : "";
  return [
    "# CI Baseline", "", "## Current fact", "", `**State:** ${stagedConclusion}. **Owner:** CI maintainers. **Next command:** \`node scripts/ci/verify_ci_baseline.mjs --records .planning/phases/226-ci-baseline-proof-semantics/226-CI-BASELINE.ndjson --rendered .planning/phases/226-ci-baseline-proof-semantics/226-CI-BASELINE.md --expected-repository szTheory/accrue\`. Evidence is the immutable Actions links below.${eventClassNote}`, "", "Privacy-safe, schema-v1 evidence. Raw logs, actors, branches, secrets, payloads, and artifact contents are not persisted.", "",
    ...
    // D-21/D-29 (231-04): a run explicitly recorded with a non-`non_run`
    // provider_state carries an honest reason, never a bare enum value with
    // no explanation. Phase 226's dataset has no such run, so this block is
    // an empty array and adds nothing to its rendering.
    ...runRows.filter((run) => run.provider_state === "skipped" && run.event_class === "workflow_dispatch").flatMap((run) => ["", `For \`workflow_dispatch\` run [${run.run_id}](${run.run_url}), the mandatory Stripe test-mode parity job (\`live-stripe\`) did not run: \`provider_state: skipped\`, because the dispatch's \`run_live_stripe\` input was explicitly set to \`false\`. A green Actions conclusion is not provider proof, and neither is this deliberate skip.`]),
    "",
...
function main() {
  ...
  if (!input || !expectedRepository) throw new Error("usage: render_ci_baseline.mjs --input records.ndjson --expected-repository owner/repository [--out baseline.md]");

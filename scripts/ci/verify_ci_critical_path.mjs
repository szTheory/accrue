...
export function renderCriticalPathEvidence(records) {
  ...
  if (records.some((record) => record.kind === "gap_budget_authorization_v3" && record.budget_id === V3_BUDGET)) {
    ...
    if (decision?.state === "kept") {
      ...
      const reservations = v3Records(records, "gap_v3_reservation");
      const consumptions = v3Records(records, "gap_v3_consumption");
      const exclusions = records.filter((record) => record.kind === "exclusion");
      const controls = records.filter((record) => record.kind === "negative_control" && Number.isSafeInteger(record.run_id));
      const observations = candidates.map((record) => `| [${record.run_id}](${record.run_url}) | ${record.duration_seconds}s | ${record.host_dag_wait_seconds}s | ${record.conclusion} | all 12 required roles success | screenshots present |`).join("\n");
      const exclusionRows = exclusions.map((record) => `| [${record.run_id}](${record.run_url}) | ${record.reason} |`).join("\n");
      const controlRows = controls.map((record) => `| [${record.run_id}](${record.run_url}) | ${record.reason || record.classification || "retained immutable negative control"} |`).join("\n");
      return `# Phase 227 critical-path kept comparison\n\n## Current fact\n\n- state: \`kept\`\n- owner: ${budget.owner}\n- PATH-02: \`satisfied\`\n- exact verification: \`node scripts/ci/verify_ci_critical_path.mjs --verify-live-actions --evidence .planning/phases/227-measured-critical-path-improvement/227-CI-CRITICAL-PATH.ndjson --rendered .planning/phases/227-measured-critical-path-improvement/227-CI-CRITICAL-PATH.md --contract .planning/phases/227-measured-critical-path-improvement/227-ci-contract.json --expected-repository szTheory/accrue --require-activation-evidence .planning/phases/227-measured-critical-path-improvement/227-CANDIDATE-PREFLIGHT.json --require-kept\`\n\n## Measured cohort\n\n- frozen Phase 226 median: \`2083s\`; frozen p95: \`2602s\`; keep threshold: \`1666s\`\n- exact sample count: \`3\`; durations: \`[1179, 1125, 1079]\` seconds; range: \`100s\`; median: \`${decision.median_seconds}s\`; maximum: \`1179s\`\n- candidate SHA: \`${activation.candidate_sha}\`; tree: \`${activation.candidate_tree}\`; workflow revision: \`${candidates[0].workflow_revision}\`; fingerprint: \`${budget.candidate_fingerprint}\`\n- workflow context: attempt-1 \`workflow_dispatch\` at \`${candidates[0].ref}\` with \`run_live_stripe: false\`; provider state is \`non_run\`, not provider proof. Separate provider evidence remains literal: \`${budget.phase228_provider_evidence}\` / \`${budget.phase228_provider_outcome}\`.\n\n| Run | Duration | Host DAG wait | Conclusion | Required vector | Required artifact |\n| --- | ---: | ---: | --- | --- | --- |\n${observations}\n\nEvery required role and the required screenshot artifact is repository-bound and live-verified. Sigra is advisory success for all three runs; the parked Admin UI ratchet is advisory failure for all three and does not alter the required vector.\n\n## Authority and safety accounting\n\n- candidate budget \`${budget.budget_id}\`: ${reservations.length}/${budget.candidate_ceiling} slots reserved and ${consumptions.length}/${budget.candidate_ceiling} consumed; no reruns, replacements, or concurrent dispatches.\n- reservations and immediate bindings: ${reservations.map((record) => `\`${record.reservation_id}\` → \`${consumptions.find((item) => item.reservation_id === record.reservation_id)?.run_id}\``).join(", ")}.\n- candidate authority: \`${decision.candidate_authority}\`; restoration authority: \`${decision.restoration_authority}\`; no restoration record exists.\n- temporary candidate ref \`phase-227-gap-dispatch-false-v3\` is removed. The exact unused inverse remains the restored workflow revision \`${decision.inverse_workflow_revision}\`; apply it only if the kept decision is formally superseded.\n- predecessors \`${budget.closed_predecessors.join("\`, \`")}\`, their runs, and all ledger bytes remain immutable and provide zero v3 observations.\n\n## Retained controls and exclusions\n\n| Negative control | Retained reason |\n| --- | --- |\n${controlRows}\n\n| Historical exclusion | Retained reason |\n| --- | --- |\n${exclusionRows}\n\nThe ledger retains all reservation, consumption, required/advisory job URLs, full artifact inventories, activation preflight binding, and recursive privacy checks. Raw logs and artifact contents remain Actions-owned; a green workflow never implies provider proof.\n`;
    }

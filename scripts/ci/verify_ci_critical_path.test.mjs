...
test("keeps v3 authority unspent until a matching preflight activation", () => {
  const history = fs.readFileSync(`${phase}/227-CI-CRITICAL-PATH.ndjson`, "utf8").trim().split("\n").map(JSON.parse).filter((record) => !["gap_budget_authorization_v3", "gap_v3_activation"].includes(record.kind));
...
test("v3 rejects duplicate consumption, incomplete kept evidence, and restoration-as-performance", () => {
  ...
  assert.equal(verifyFinalDecision([...history, budget, activation, ...reservations, ...consumptions, ...terminals, kept], contract).state, "kept");
  assert.throws(() => verifyFinalDecision([...history, budget, activation, ...reservations, ...consumptions, consumptions[0], ...terminals, kept], contract), /consumed twice|dispatched twice/);
  assert.throws(() => verifyFinalDecision([...history, budget, activation, ...reservations, ...consumptions, terminals[0], terminals[1], kept], contract), /candidate IDs differ|exactly three/);
  const failed = terminal(3, "nonqualifying");
  assert.throws(() => verifyFinalDecision([...history, budget, activation, ...reservations, ...consumptions, terminals[0], terminals[1], failed, kept], contract), /nonqualifying candidate/);
  ...
  assert.throws(() => verifyFinalDecision([...history, budget, activation, ...reservations, ...consumptions, ...terminals, restoreReservation, restoreConsumption, restoration, kept], contract), /cannot include restoration/);
});
